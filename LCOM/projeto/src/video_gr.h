#ifndef __VIDEO_GR_H
#define __VIDEO_GR_H





/** @defgroup video_gr video_gr
 * @{
 *
 * Functions for outputing data to screen in graphics mode
 */

/**
 * @brief Initializes the video module in graphics mode
 * 
 * Uses the VBE INT 0x10 interface to set the desired
 *  graphics mode, maps VRAM to the process' address space and
 *  initializes static global variables with the resolution of the screen, 
 *  and the number of colors
 * 
 * @param mode 16-bit VBE mode to set
 * @return Virtual address VRAM was mapped to. NULL, upon failure.
 */
void *vg_init(unsigned short mode);

 /**
 * @brief Returns to default Minix 3 text mode (0x03: 25 x 80, 16 colors)
 * 
 * @return 0 upon success, non-zero upon failure
 */
int vg_exit(void);
/**
* @brief Returns pointer to video_mem
*
* @return video_mem
*/
char* getVideoMem();

/**
* @brief paints a pixel at the given coordinates using color)
*
* @param x x coordinate of pixel
* @param y y coordinate of pixel
* @ param color color
*/
void paint_pixel(unsigned short x, unsigned short y, unsigned long color) ;
/**
* @brief draws a square where x and y are top left coordinates using color
* @param x x coordinate of top left corner
* @param y y coordinate of top left corner
* @ param color color
* */
void draw_square(unsigned short x, unsigned short y, unsigned short size,
		unsigned long color) ;
/**
* @brief paints the screen of a given color
* @ param color color
* */
void clear_screen(unsigned long color);
/**
* @brief returns  HRES
* */
unsigned int getXResolution();
/**
* @brief returns VRES
* */
unsigned int getYResolution();
/**
* @brief returns bitsPerPixel
* */
unsigned int getBitsperpixel();

 /** @} end of video_gr */
 
#endif /* __VIDEO_GR_H */
